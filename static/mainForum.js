...
async function refreshForums() {
    ...
    if (Forums.length > 0) {
        document.getElementById("forumHead").innerHTML = "<h4>Forum Discussion</h4>";
    } else {
        document.getElementById("forumHead").innerHTML = "<h4>There's no forum yet</h4>";
    }
...
function addForums() {
    fetch("{% url 'bibliofilia:add_Forum_ajax' %}", {
        method: "POST",
